...
architecture Behavioral of red_light_green_light is
    ...
    signal db_count    : integer range 0 to 999999 := 0; 
    signal btn_stable  : STD_LOGIC := '0';   
    signal btn_pressed : STD_LOGIC := '0';   

    constant DB_CYCLES : integer := 999999;  
    ...
    -- Bonus: LFSR-based randomized Green Light duration
    --
    --   A free-running 8-bit Galois LFSR (taps 8,6,5,4 per
    --   standard table) runs continuously at 100 MHz.
    --   Its value is sampled once per RED-phase processing
    --   to add a bounded offset to the nominal duration.
    --
    --   Nominal duration  : nom  = (prev * 3) / 4   (25% reduction)
    --   Offset range      : ±(nom / 4), i.e. 25% of nominal
    --   Actual duration   : nom + offset, clamped to [1, 63]
    --
    --   The 2 LSBs of the LFSR select the offset:
    --     "00" =>  0          (no change)
    --     "01" => +nom/4      (longer)
    --     "10" => -nom/4      (shorter)
    --     "11" =>  0          (no change, keeps distribution symmetric)
    signal lfsr        : STD_LOGIC_VECTOR(7 downto 0) := "10101100"; 
    signal nom_time    : integer range 0 to 63 := 6;   
    signal blink       : STD_LOGIC := '0';
    signal blink_count : integer range 0 to 199 := 0;
...
begin
    ...
    begin
        if rising_edge(clk) then
            clk_en_1hz  <= '0';
            clk_en_fast <= '0';
            if count1 = 99999999 then
                count1     <= 0;
                clk_en_1hz <= '1';
            else
                count1 <= count1 + 1;
            end if;
            if count2 = 99999 then
                count2      <= 0;
                clk_en_fast <= '1';
            else
                count2 <= count2 + 1;
    ...
    --   Runs every clock cycle at 100 MHz.
    ...
    -- Button debouncer
    --   1. Two-FF synchroniser to cross from async button domain.
    --   2. Stable-period counter: the raw sample must remain
    --      unchanged for DB_CYCLES consecutive clock cycles
    --      before btn_stable is updated.
    --   3. btn_pressed is a single-cycle rising-edge pulse.
    process(clk, reset)
    begin
        if reset = '1' then
            btn_sync0   <= '0';
            ...
            if clk_en_1hz = '1' then
                ...
                if state_reg = GREEN then
                    ...
                    if red_processed = '0' then
                        ...
                        -- Step 1: compute nominal (25% reduction)
    ...
    -- Blink at ~5 Hz (200 fast ticks × 1 ms = 200 ms half-period)
    process(clk)
    begin
        if rising_edge(clk) then
            if clk_en_fast = '1' then
                if blink_count = 199 then
                    blink_count <= 0;
                    blink       <= not blink;
                else
